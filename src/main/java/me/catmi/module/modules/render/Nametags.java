...
import me.catmi.module.modules.hud.ColorMain;
...
public class Nametags extends Module {
	...
	private void renderItemName(ItemStack itemStack, int x, int y) {
		...
		FontUtils.drawStringWithShadow(HUD.customFont.getValue(), s2, -FontUtils.getStringWidth(HUD.customFont.getValue(), s2) /2, y, new CMColor(255,255,255));
	...
	private void renderEnchants(ItemStack itemStack, int x, int y) {
		...
		while (iterator.hasNext()) {
			...
			if ((enchantment = iterator2.next()) == null) {
				...
				if (enchantnames.getValue()) {
					FontUtils.drawStringWithShadow(HUD.customFont.getValue(), this.stringForEnchants(enchantment3, EnchantmentHelper.getEnchantmentLevel(enchantment3, itemStack)), (x * 2), y, new CMColor(255,255,255));
				}
				else {
					return;
				}
		...
		if (itemStack.getItem().equals(Items.GOLDEN_APPLE) && itemStack.hasEffect()) {
			FontUtils.drawStringWithShadow(HUD.customFont.getValue(), "God", (x * 2), y,new CMColor(195,77,65));
		}
	}
	...
	private void renderItemDurability(final ItemStack itemStack, final int x, final int y) {
		...
		GlStateManager.disableDepth();
		FontUtils.drawStringWithShadow(HUD.customFont.getValue(),new StringBuilder().insert(0, (int) (n3 * 100.0f)).append('%').toString(), (x * 2), y, new CMColor((int) (red * 255), (int) (green * 255), 0));
		GlStateManager.enableDepth();
		final float n6 = 2.0f;
		final int n7 = 2;
	...
	private void renderNametags(final EntityPlayer entityPlayer, final double n, double distance, final double n2) {
		...
		else color=new CMColor(0,0,0,51);
		drawBorderedRectReliant((float) (-n4 - 1), (float) (-mc.fontRenderer.FONT_HEIGHT), (float) (n4 + 2), 1.0f, 1.8f, new CMColor(0,4,0,85), color);
	...
	private CMColor renderPing(final EntityPlayer entityPlayer) {
	...
	public static void drawBorderedRectReliant(final float x, final float y, final float x1, final float y1, final float lineWidth, final CMColor inside, final CMColor border) {
		enableGL2D();
	...
	public static void drawRect(final float x, final float y, final float x1, final float y1, final CMColor color) {
		enableGL2D();
		color.glColor();
		drawRect(x, y, x1, y1);
		disableGL2D();
