...
public class HoleESP extends Module{
	...
	public void setup(){
		...
		bedrockColor=registerColor("Bedrock Color","BedrockColor",new CMColor(0,255,0));
		otherColor=registerColor("Obsidian Color","ObsidianColor",new CMColor(255,0,0));
	}
	...
	private CMColor getColor (boolean isBedrock, int alpha) {
		CMColor c;
		if (isBedrock) c=bedrockColor.getValue();
		else c=otherColor.getValue();
		return new CMColor(c,alpha);
	}
	...
	private void drawBox(BlockPos blockPos, boolean isBedrock) {
		if (type.getValue().equalsIgnoreCase("Fill") || type.getValue().equalsIgnoreCase("Both")) {
			CMColor color=getColor(isBedrock,50);
			...
			if (mode.getValue().equalsIgnoreCase("Air")) {
				if (this.flatOwn.getValue() && blockPos.equals(new BlockPos(mc.player.posX, mc.player.posY, mc.player.posZ)))
					CatmiTessellator.drawBox(blockPos, color, GeometryMasks.Quad.DOWN);
				else
					CatmiTessellator.drawBox(blockPos, color, GeometryMasks.Quad.ALL);
			}
		}
	}
	...
	private void drawDownBox(BlockPos blockPos, boolean isBedrock){
		if (type.getValue().equalsIgnoreCase("Fill") || type.getValue().equalsIgnoreCase("Both")) {
			...
			if (mode.getValue().equalsIgnoreCase("Ground")) {
				CatmiTessellator.drawDownBox(blockPos, color, GeometryMasks.Quad.ALL);
			}
		}
	}
	...
	private void drawOutline(BlockPos blockPos, int width, boolean isBedrock) {
		CMColor color=getColor(isBedrock,255);
		if (type.getValue().equalsIgnoreCase("Outline") || type.getValue().equalsIgnoreCase("Both")) {
			if (mode.getValue().equalsIgnoreCase("Air")) {
				if (this.flatOwn.getValue() && blockPos.equals(new BlockPos(mc.player.posX, mc.player.posY, mc.player.posZ))) {
					CatmiTessellator.drawBoundingBoxBottom2(blockPos, width, color);
				} else {
					CatmiTessellator.drawBoundingBoxBlockPos(blockPos, width, color);
				}
			}
			if (mode.getValue().equalsIgnoreCase("Flat")) {
				CatmiTessellator.drawBoundingBoxBottom2(blockPos, width, color);
			...
			if (mode.getValue().equalsIgnoreCase("Ground")) {
				CatmiTessellator.drawBoundingBoxBlockPos2(blockPos, width, color);
			}
		}
	}
}