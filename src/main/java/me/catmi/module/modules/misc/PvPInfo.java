...
import me.catmi.event.events.TotemPopEvent;
			...
			for (Entity e : players){
				if (e instanceof EntityPlayer && !e.getName().equalsIgnoreCase(mc.player.getName())){
					if (!knownPlayers.contains(e)){
						knownPlayers.add(e);
						Command.sendClientMessage(getTextColor() + e.getName() + " has been spotted thanks to Catmi!");
					}
				}
			}
		} catch (Exception e){
	...
	public Listener<PacketEvent.Receive> totemPopListener = new Listener<>(event -> {
		...
		if (event.getPacket() instanceof SPacketEntityStatus){
			...
			if (packet.getOpCode() == 35){
				...
				Catmi.EVENT_BUS.post(new TotemPopEvent(entity));
			}
		}
