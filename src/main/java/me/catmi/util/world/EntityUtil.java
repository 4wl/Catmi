...
public class EntityUtil{
	...
	public static float calculateDamage(double posX, double posY, double posZ, Entity entity) {
		...
		if (entity instanceof EntityLivingBase) {
			finald = EntityUtil.getBlastReduction((EntityLivingBase)entity, EntityUtil.getDamageMultiplied(damage), new Explosion((World)EntityUtil.mc.world, null, posX, posY, posZ, 6.0f, false, true));
		}
		return (float)finald;
	}
	...
	public static float getDamageInPercent(ItemStack stack) {
		return (float)EntityUtil.getItemDamage(stack) / (float)stack.getMaxDamage() * 100.0f;
	}
	...
	public static int getRoundedDamage(ItemStack stack) {
		return (int)EntityUtil.getDamageInPercent(stack);
	}
