...
public class CFontRenderer extends CFont {
	...
	public float drawStringWithShadow(String text, double x, double y, CMColor color) {
	...
	public float drawStringWithShadow2(String text, double x, double y, int color) {
		float shadowWidth = Catmi.fontRenderer2.drawString(text, x + 1D, y + 1D, color, true);
		return Math.max(shadowWidth, Catmi.fontRenderer2.drawString(text, x, y, color, false));
	}

	public float drawString(String text, float x, float y, CMColor color) {
		return drawString(text, x, y, color, false);
	}

	public float drawCenteredStringWithShadow(String text, float x, float y, CMColor color) {
		return drawStringWithShadow(text, x - getStringWidth(text) / 2, y, color);
	}

	public float drawCenteredString(String text, float x, float y, CMColor color) {
		return drawString(text, x - getStringWidth(text) / 2, y, color);
	}

	public float drawString(String text, double x, double y, CMColor gsColor, boolean shadow) {
		x -= 1;
		y -= 2;
		CMColor color=new CMColor(gsColor);
		if (text == null) return 0.0F;
		if (color.getRed()==255 && color.getGreen()==255 && color.getBlue()==255 && color.getAlpha()==32) color=new CMColor(255,255,255);
		if (color.getAlpha()<4) color=new CMColor(color,255);
		if (shadow) color=new CMColor(color.getRed()/4,color.getGreen()/4,color.getBlue()/4,color.getAlpha());
