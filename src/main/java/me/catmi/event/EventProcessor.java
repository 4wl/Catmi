package me.catmi.event;

import me.catmi.event.events.PacketEvent;
import me.catmi.event.events.PlayerJoinEvent;
import me.catmi.event.events.PlayerLeaveEvent;
...
public class EventProcessor {
	 ...
	 public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Keyboard.getEventKeyState()) {
		  ...
		  ModuleManager.onBind(Keyboard.getEventKey());
			//Macro
		   Catmi.getInstance().macroManager.getMacros().forEach(m -> {
	...
	@SubscribeEvent
	public void onRenderBlockOverlay(RenderBlockOverlayEvent event){ Catmi.EVENT_BUS.post(event); }
							...
							new Thread(() -> {
								...
								if (name != null) {
									if (mc.player != null && mc.player.ticksExisted >= 1000)
										Catmi.EVENT_BUS.post(new PlayerJoinEvent(name));
								}
							}).start();
						}
					}
					...
					for (SPacketPlayerListItem.AddPlayerData playerData : packet.getEntries()) {
						if (playerData.getProfile().getId() != mc.session.getProfile().getId()) {
							new Thread(() -> {
								...
								if (name != null) {
									if (mc.player != null && mc.player.ticksExisted >= 1000)
										Catmi.EVENT_BUS.post(new PlayerLeaveEvent(name));
								}
							}).start();
						}
					}
