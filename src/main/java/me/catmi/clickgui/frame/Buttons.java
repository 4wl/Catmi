...
import me.catmi.clickgui.buttons.*;
...
public class Buttons extends Component {
	...
	public Buttons(final Module mod, final Frames parent, final int offset) {
		...
		this.open = false;
		int opY = offset + 16;
		if (Catmi.getInstance().settingsManager.getSettingsForMod(mod) != null && !Catmi.getInstance().settingsManager.getSettingsForMod(mod).isEmpty()) {
			for (final Setting s : Catmi.getInstance().settingsManager.getSettingsForMod(mod)) {
				switch (s.getType()) {
					case MODE:
