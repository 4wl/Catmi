...
public class ColorComponent extends Component {
    ...
    public void updateComponent(final int mouseX, final int mouseY) {
        ...
        if (this.dragging) {
			CMColor c=set.getColor();
			if (ColorMain.colorModel.getValue().equalsIgnoreCase("RGB")) {
				int newValue;
				if (diff == 0.0) newValue=0;
				else newValue = (int)roundToPlace(diff/100.0*255, 2);
				if (mouseY-this.y>=32 && mouseY-this.y<48) {
					set.setValue(set.getRainbow(),new CMColor(newValue,c.getGreen(),c.getBlue()));
				} else if (mouseY-this.y>=48 && mouseY-this.y<64) {
					set.setValue(set.getRainbow(),new CMColor(c.getRed(),newValue,c.getBlue()));
				} else if (mouseY-this.y>=64 && mouseY-this.y<80) {
					set.setValue(set.getRainbow(),new CMColor(c.getRed(),c.getGreen(),newValue));
				}
			} else {
				float newValue=(float)(diff/100.0);
				if (mouseY-this.y>=32 && mouseY-this.y<48) {
					set.setValue(set.getRainbow(), CMColor.fromHSB(newValue,c.getSaturation(),c.getBrightness()));
				} else if (mouseY-this.y>=48 && mouseY-this.y<64) {
					set.setValue(set.getRainbow(), CMColor.fromHSB(c.getHue(),newValue,c.getBrightness()));
				} else if (mouseY-this.y>=64 && mouseY-this.y<80) {
					set.setValue(set.getRainbow(), CMColor.fromHSB(c.getHue(),c.getSaturation(),newValue));
				}
			}
        }
    }
